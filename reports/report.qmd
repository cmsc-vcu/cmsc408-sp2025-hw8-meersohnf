...
select
  ...
  round(sum(case when `Income Group` = 'High income'         then 1 else 0 end)
        / (select count(*) from wdi_country) * 100, 2) as high_income_pct,
  round(sum(case when `Income Group` = 'Upper middle income' then 1 else 0 end)
        / (select count(*) from wdi_country) * 100, 2) as upper_middle_income_pct,
  round(sum(case when `Income Group` = 'Lower middle income' then 1 else 0 end)
        / (select count(*) from wdi_country) * 100, 2) as lower_middle_income_pct,
  round(sum(case when `Income Group` = 'Low income'          then 1 else 0 end)
        / (select count(*) from wdi_country) * 100, 2) as low_income_pct,
  round(sum(case when `Income Group` is null                 then 1 else 0 end)
        / (select count(*) from wdi_country) * 100, 2) as other_pct
from
  wdi_country
group by
  region
...
select
  ...
  round(count(*) / (select count(*) from wdi_country) * 100, 2) as percent_of_total
from
  wdi_country
group by
  `Income Group`
